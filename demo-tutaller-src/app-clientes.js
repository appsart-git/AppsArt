...
// (un mismo auto puede volver con otro dueño con el tiempo). El "próximo service" que se ve acá
// se calcula en app-fidelizacion.js a partir del último trabajo guardado.
...
function recompraPillHtml(clienteId){
  const info = proximoServicioCliente(clienteId);
  if(!info) return `<span class="pill pill-muted">Sin historial</span>`;
  if(info.estado === 'vencido') return `<span class="pill pill-red">Vencido hace ${Math.abs(info.dias)}d</span>`;
  if(info.estado === 'porVencer') return `<span class="pill pill-amber">Vence en ${info.dias}d</span>`;
  return `<span class="pill pill-ok">Al día</span>`;
}
...
function renderClientes(){
  ...
  document.getElementById('main').innerHTML = `
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>Nombre</th><th>Teléfono</th><th>Vehículos</th><th>Trabajos</th><th>Recompra</th><th></th></tr></thead>
        <tbody>
          ...
          }).join('') : `<tr><td colspan="6" class="empty">Todavía no hay clientes cargados.</td></tr>`}
        </tbody>
      </table>
    </div>`;
}
...
function clienteDetailHtml(id){
  ...
  const info = proximoServicioCliente(id);
  return `
    ...
    <div style="margin:14px 0;">${recompraPillHtml(id)} ${info ? `<span class="muted small" style="margin-left:8px;">Próximo service estimado: ${fmtDate(info.fecha)}</span>` : ''}</div>
    <div style="margin:0 0 10px; display:flex; gap:8px; flex-wrap:wrap;">
      ...
      ${c.telefono ? `<a class="btn btn-sm btn-wa" href="${waLink(c.telefono, fillTemplate(state.negocio.mensajeRecordatorio, {nombre:c.nombre.split(' ')[0], vehiculo: vs[0]?vehiculoLabel(vs[0]):'tu vehículo', taller: state.negocio.nombre}))}" target="_blank" rel="noopener">💬 WhatsApp</a>` : ''}
    ...
    <h3 style="font-size:14px; margin-top:18px;">Historial de trabajos <span class="muted small">(por patente, con fecha y repuestos)</span></h3>
...
/* Buscador de clientes reutilizado desde el informe y desde seguimiento (recepción rápida) para
   vincular a un cliente/vehículo existente sin tener que ir a la sección Clientes. */
function buscadorClienteListHtml(){
  ...
  return list.length ? list.map(c=>{
    const vs = vehiculosDeCliente(c.id);
    return `<div class="row-card" data-action="${state._buscadorClienteTarget||'elegirClienteInforme'}" data-id="${c.id}" role="button" tabindex="0">
