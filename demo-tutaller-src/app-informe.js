...
// Arma un informe de una página con foto, logo y detalle de reparación para mandar por WhatsApp
// (esto ya lo vendía Martín suelto a 3 talleres) y además guarda el trabajo en la base (colección
// `trabajos`, atada a clienteId+vehiculoId) para que quede buscable desde Clientes y Trabajos, y
// alimente el cálculo de "próximo service" de la sección Recompra. La foto NO se guarda en la base
// — solo se usa para armar el PDF/compartir de ese momento.
...
/* ---------- ícono por tipo de repuesto (frenos, motor, suspensión, genérico...) ---------- */
function iconBadge(inner){
  return `<svg width="22" height="22" viewBox="0 0 24 24"><circle cx="12" cy="12" r="12" fill="#c8202b"/>${inner}</svg>`;
}
const ICONS = {
  freno: iconBadge('<circle cx="12" cy="12" r="5.5" stroke="#fff" stroke-width="1.5" fill="none"/><circle cx="12" cy="12" r="1.6" fill="#fff"/><path d="M12 6.5v2M12 15.5v2M6.5 12h2M15.5 12h2" stroke="#fff" stroke-width="1.4" stroke-linecap="round"/>'),
  ...
  correa: iconBadge('<circle cx="9" cy="12" r="3" stroke="#fff" stroke-width="1.5" fill="none"/><circle cx="16" cy="12" r="2" stroke="#fff" stroke-width="1.5" fill="none"/><path d="M11.8 10.3L14.4 10.7M11.8 13.7L14.4 13.3" stroke="#fff" stroke-width="1.3" stroke-linecap="round"/>'),
...
function pickIcon(name){
  ...
  if(/freno|pastilla|disco|campana/.test(n)) return ICONS.freno;
  if(/sensor|sonda|lambda|ox[íi]geno/.test(n)) return ICONS.sensor;
  if(/correa|kit distribuci|polea|tensor/.test(n)) return ICONS.correa;
  ...
  <div class="layout">
    <div class="form-col">
      ...
      <div class="sticky-actions">
        ...
        <div class="print-hint">Guardar trabajo lo suma al historial del cliente y actualiza su próximo service. El PDF sirve para enviarlo por WhatsApp.</div>
      </div>
    </div>
        ...
        <div id="report">
          <div class="r-header">
            ...
            <div class="r-header-info">
              <div class="r-logo-strip"><img src="./logo-tutaller.png"></div>
          ...
          <div class="r-footer">
            ...
            <span>MANTENIMIENTO Y REPARACIÓN AUTOMOTRIZ A MEDIDA</span>
  ...
  document.getElementById('shareBtn').onclick = () => withBusyButton(document.getElementById('shareBtn'), 'Generando…', async () => {
    try{
      ...
      if(navigator.canShare && navigator.canShare({files:[file]})){
        await navigator.share({files:[file], title:'Informe ' + state.negocio.nombre, text:`Informe de ${getMarcaValue()||''} ${getModeloValue()||''}`.trim()});
      } else {
  ...
  state.informe.repuestos.forEach((r, i) => {
    ...
    card.innerHTML = `
      ...
      <div style="display:flex; gap:10px; align-items:flex-start;">
        ...
        <div style="flex:1;">
          <div class="field" style="margin-bottom:8px;"><input placeholder="Nombre del repuesto (ej: Pastillas de freno)" data-field="nombre" data-i="${i}" value="${esc(r.nombre||'')}"></div>
          <div class="row2">
...
function reportFileName(){
  return 'informe-tutaller-' + (getModeloValue() || 'vehiculo').toLowerCase().replace(/[^a-z0-9]+/g,'-') + '.pdf';
}
...
async function guardarTrabajo(){
  ...
  try{
    ...
    doneSaving();
    toast('Trabajo guardado. Próximo service recalculado.');
  }catch(e){ saveError(e); }
}

/* ---------- vincular cliente existente desde el buscador ---------- */
Object.assign(actions, {
  buscarClienteInforme(){ state._buscadorClienteQuery=''; state._buscadorClienteTarget='elegirClienteInforme'; openModal(buscadorClienteHtml()); },
...
/* ---------- carga un trabajo del historial (o un cliente/vehículo de una orden en curso) como el
   informe activo — reutilizado por Trabajos y por "Generar informe" desde Seguimiento ---------- */
...
function cargarClienteVehiculoComoInforme(clienteId, vehiculoId){
  const c = state.clientes.find(x=>x.id===clienteId);
  const v = state.vehiculos.find(x=>x.id===vehiculoId);
  const inf = blankInforme();
  inf.clienteId = clienteId || null;
  inf.vehiculoId = vehiculoId || null;
  if(c){ inf.fields.responsable = c.nombre||''; inf.fields.telefono = c.telefono||''; inf.fields.direccion = c.direccion||''; }
  if(v){ Object.assign(inf.fields, vehiculoToFields(v)); }
  state.informe = inf;
}
